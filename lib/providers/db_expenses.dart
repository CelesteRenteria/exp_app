...
class DBExpenses{
  ...
  Future<List<ExpensesModel>>getExpensesByDate(int month, int year)async{
    final db = await database;
    final response = await db.query('Expenses',where:'month = ? and year = ?',
    whereArgs: [month, year]);
    List<ExpensesModel> data = response.isNotEmpty? response.map((e) => ExpensesModel.fromJson(e)).toList() : [];
    return data;
  }

  Future<int> updateExpense(ExpensesModel exp)async{
    final db = await database;
...
Future<List<EntriesModel>>getEntriesByDate(int month, int year)async{
    ...
    whereArgs: [month, year]);
    List<EntriesModel> data = response.isNotEmpty? response.map((e) => EntriesModel.fromJson(e)).toList() : [];
    return data;
  }
