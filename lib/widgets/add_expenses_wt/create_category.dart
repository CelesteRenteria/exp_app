...
super.initState();
  ...
  Widget build(BuildContext context) {
    final fList = context.watch<ExpensesProvider>().data;
    ...
    addCategory() {
      if (contain.isNotEmpty) {
        Fluttertoast.showToast(msg:'Ya existe esa categoría',
        backgroundColor: Colors.red, fontSize:20, gravity: ToastGravity.CENTER);
      }
      else if (widget.fModel.category.isNotEmpty) {
        exProvider.addNewFeatures(
          widget.fModel);
        Navigator.pop(context);
        Fluttertoast.showToast(msg:'Categoría agregada',
        backgroundColor: Colors.red, fontSize:20,  gravity: ToastGravity.CENTER);
      } else {
...
Fluttertoast.showToast(msg:'No olvides nombrar la categoría',
        backgroundColor: Colors.red, fontSize:20, gravity: ToastGravity.CENTER);
      }
    }
        ...
        child: Column(
          children: [
            Container(
              ...
              child: ListTile(
                trailing: const Icon(Icons.text_fields_outlined, size: 35),
                title: TextFormField(
            ...
            children: [
              MaterialColorPicker(
                ...
                onColorChange: (Color selectedColor) {
                  var hexColor =
                      '#${selectedColor.value.toRadixString(16).substring(2, 8)}';
                  setState(() {
